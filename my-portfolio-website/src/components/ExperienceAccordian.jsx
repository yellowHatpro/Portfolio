                ...
                experienceContent.map((item, i) => (
                    <div key={item.title}>
                        <div
                            ...
                            <div>{item.title}</div>
                            <div
                                className={"max-h-0 group-hover:bg-yellow-300 transition-all duration-500 group-hover:text-black flex flex-row justify-center items-center "}>
                                {openIndex === i ? <a target={"_blank"} href={item.link} rel="noreferrer">
                                    <TfiArrowTopRight className={"transition-all duration-200"}/>
                                </a>:
                                    <TfiArrowDown className={"transition-all duration-200"}/>}</div>
                        </div>
                        <div
